...
const filePath = 'components/my-cosmetics/PouchDecorateBottomSheet.tsx';
...
function WappenGrid({
  ...
  return (
    <motionlessTabRow className="grid grid-cols-4 gap-x-3 gap-y-3">
      {wappens.map((wappen) => {
...
const fixed = content
  ...
  .replace(/motionlessTabRow/g, 'div');

fs.writeFileSync(filePath, fixed, 'utf8');
console.log('Wrote', filePath, 'bytes:', fixed.length);
